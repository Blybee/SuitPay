...
- Exigen atestación de la aplicación. Una llamada desde fuera de la aplicación se rechaza.
...
3. Abre una transacción que: busca el comprobante con esa clave; si existe, termina y devuelve su estado sin emitir; si no existe, consume el correlativo de la serie del vendedor, crea el comprobante en estado `reclamado` y, cuando venga de una cotización, la marca como `convertida` en el mismo acto.
...
**Petición**: `medioUrl` del archivo cargado, `modo` (`validar` o `publicar`).

**Comportamiento**: en modo validar, interpreta el archivo y devuelve el resumen sin escribir nada: cuántos productos se reconocieron, cuáles presentan conflicto y, si ya hay catálogo publicado, qué productos son nuevos, cuáles cambian de precio y cuáles desaparecen. En modo publicar, escribe el catálogo **en una sola escritura** e incrementa su versión.

Escribir los ~500 productos como una única escritura no es una optimización menor: 500 escrituras individuales sobre el mismo recurso serían más caras y chocarían con los límites de escritura por documento.
...
## `reconciliarEmisiones` (programada)

Existe para que ninguna venta quede en un estado desconocido, y es la contrapartida obligatoria de la prohibición de reintentar a ciegas.

**Disparo**: periódica, y también a demanda desde la administración.

**Comportamiento**: recorre los comprobantes en estado `indeterminado`. Para cada uno consulta al proveedor por su serie y número. Si el documento existe, adopta su estado real. Si no existe, la venta puede volver a emitirse de forma segura. Si el número no era conocido —porque el proveedor lo asigna y no lo aceptó explícito— aplica el sondeo acotado descrito en la decisión 4 de `research.md`, comparando cliente, total y fecha. Si la ventana se agota sin certeza, mueve el comprobante a `requiere_intervencion`.

**Nunca emite**. Solo averigua y corrige el estado.

---

## `procesarPendientes` (programada)

**Comportamiento**: recorre los comprobantes en estado `pendiente` —los que quedaron esperando porque el proveedor no respondía— y completa su emisión. Al lograrlo, deja el documento en condiciones de hacerse llegar al cliente con el contacto recogido. Si el resultado es un rechazo, mueve el comprobante a `requiere_intervencion`, porque el cliente ya se fue con mercadería y con un documento interno.
