...
| `cotizaciones/{id}` | leer, crear, editar y **borrar** las pendientes | leer, crear, editar, borrar pendientes | leer | Canal `general`: la conversión las borra el backend en la transacción de emisión (FR-019). Canal `vecino`: la conversión actualiza `generacionPedido` y vacía líneas (FR-035a). El cliente puede borrar pendientes tras confirmación en UI (FR-019a). En canal `vecino` se admite `telefonoVecino` y editar `aliasVecino`. `diaCivilLineas` y `totalDeudas` son mutables por el cliente (FR-035g). El contador de generación solo lo incrementa el backend. |
| `cotizaciones/{id}/deudasPorDia/{AAAA-MM-DD}` | leer, crear, editar, borrar | leer, crear, editar, borrar | leer | Historial operativo de listas no emitidas (FR-035g). El id es el día civil Lima. `generacion` nace en 0; el cliente no la incrementa. La emisión las borra el backend. |
...
- Al editar una cotización: el estado sigue siendo `pendiente`, no se altera la autoría, el `canal` ni `generacionPedido`; las claves afectadas MUST pertenecer a `{aliasVecino, telefonoVecino, cliente, lineas, total, actualizadoEn, diaCivilLineas, totalDeudas}`. El alias, el teléfono y la instantánea embebida de cliente sí pueden cambiar en canal vecino. Un documento con campos residuales de un esquema previo no bloquea ese update. El contador de generación solo lo incrementa el backend (FR-035a).
- Al escribir `cotizaciones/{id}/deudasPorDia/{dia}`: `{dia}` tiene forma `AAAA-MM-DD`, `fecha` coincide con el id, `lineas` es un arreglo, `total` es entero y `generacion` es `0` en create; en update no se incrementa `generacion`.
