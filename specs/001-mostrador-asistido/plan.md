...
**Constraints**: ningún secreto en el cliente; la búsqueda de productos no depende de servicios externos (FR-007); el pedido en curso sobrevive a la pérdida de conexión (FR-015); dictado y foto no envían ficha de clientes (FR-045); el PDF de requerimiento (FR-061) es la excepción documentada del principio IV; el cliente no puede escribir comprobantes directamente en la base de datos.
...
| IV | Datos de clientes fuera de los servicios de IA (NO NEGOCIABLE) | Dictado y foto: tráfico por `interpretarCaptura`, identidad en el sistema. PDF de requerimiento (FR-061, constitución v1.1.0): el archivo MAY ir al modelo; catálogo y `clientes/{id}` no. | **pass** |
