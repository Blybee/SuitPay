...
- **FR-019**: Al convertir una cotización **general** en boleta, factura o nota de venta, el sistema MUST eliminar en duro el documento de cotización en la misma transacción que crea el comprobante. Si la cotización ya no existe, MUST impedir una segunda conversión.
...
- **FR-035a**: La conversión de una cotización de vecino a boleta, factura o nota de venta MUST ocurrir en la misma transacción que crea el comprobante. MUST NOT borrar el documento del vecino. MUST incrementar `generacionPedido` y vaciar `lineas`/`total`, conservando alias, teléfono y cliente. Si la generación enviada no coincide, MUST impedir la emisión (`cotizacion_ya_usada`). El modal MUST ofrecer «Comprobantes» y «Nuevo pedido»: el segundo MUST soltar el origen obsoleto (cotización y generación) y la clave de idempotencia, sin vaciar líneas ni cliente.
...
- **FR-035g**: El pedido vivo de un vecino es el del día civil America/Lima. Si al cruzar ese día quedan líneas sin emitir, el sistema MUST archivarlas como deuda de esa fecha (`deudasPorDia/{AAAA-MM-DD}`) y vaciar el pedido vivo, sin cron: al abrir el vecino, al persistir y al recuperar el foco. El pie del tab MUST sustituir el total duplicado por un pill `Deudas {monto}` con conmutador de vista (ojito). El monto por omisión es `totalDeudas` del padre; con fechas marcadas, la suma de esas fechas. El detalle de deudas MUST cargarse bajo demanda al abrir el ojito, solo del vecino activo. Cada día muestra fecha, casilla, total, líneas (solo lectura) y tacho con confirmación en diálogo. «Convertir en documento» en vista deudas MUST exigir al menos una fecha, fusionar líneas por código (suma de cantidades; precio del día más reciente) y cargar Pedido con origen de deudas, sin ligar `generacionPedido` del vivo. La emisión MUST consumir esas fechas en la misma transacción y MUST NOT vaciar el pedido vivo. Captura del pill de alias: vista hoy = lista viva + total; vista deudas con selección = fechas marcadas fusionadas + total de la selección; sin selección = todos los días + total general.
...
**Nota de alcance (crédito/cobro)**: la emisión a crédito con fecha de vencimiento, el registro de cobro y la consulta de pendientes de cobro **no forman parte de esta entrega**. FR-035g es historial operativo de listas no emitidas por día civil, no un módulo de cobranzas: el comprobante nace cuando el vendedor convierte y confirma en Pedido.
