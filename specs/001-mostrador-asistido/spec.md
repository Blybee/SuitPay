...
- **FR-060b**: La lista MUST persistirse en Firestore por día laboral (`listasRequerimiento/{uid}/dias/{AAAA-MM-DD}`) para sincronizar entre dispositivos del mismo vendedor, con `caducaEn` a 1 semana. MUST ofrecer Exportar PDF y compartir ese PDF por WhatsApp (documento interno, no un comprobante).
- **FR-060c**: El tab Lista MUST mostrar pills de lunes a sábado (abreviados; fecha corta en teléfono) de la semana actual en America/Lima, abriendo en el día actual. Cada día MUST leerse bajo demanda al pulsar su pill, nunca al arrancar. Las altas nuevas van al día actual.
